...
void loop() {
  ...
  for(int k=0;k<15;k++) { //Random sequence
    seq[k] = random(0,4);
    btnApertar[k] = seq[k];
  }
  ...
  while(rodaJogo == true) { //timer_1
    ...
    if(btn == 99)//Checks the end of the game
      break;
    else
      posiAtual = 0;
    
    if(posiFinal == 14) {//Checks if the player has finished the sequence
      for(int j=0;j<10;j++) {
        for(int i=0;i<4;i++)
          digitalWrite(leds[i],HIGH);
